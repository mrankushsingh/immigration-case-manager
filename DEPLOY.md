...
2. **Deploy:**
   ...
   - For backend, it will use the `railway.json` config
   - Add Environment Variable:
...
The app uses in-memory storage. Data will reset when the server restarts. For production, consider:
- Adding a database (PostgreSQL, MongoDB, etc.)
- Using a free database service (Supabase, MongoDB Atlas, etc.)
